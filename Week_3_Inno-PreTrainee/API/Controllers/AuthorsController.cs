...
namespace Week_3_Inno_PreTrainee.Presentation.Controllers
{
    ...
    public class AuthorsController(
            IServiceAuthor service,
            IValidator<AuthorForCreationDto> putAuthorCreationValidator,
            IValidator<AuthorForUpdateDto> putAuthorUpdateValidator
    ) : ControllerBase
    {
        private readonly IServiceAuthor _service = service;
        private readonly IValidator<AuthorForCreationDto> _putAuthorCreationValidator = putAuthorCreationValidator;
        private readonly IValidator<AuthorForUpdateDto> _putAuthorUpdateValidator = putAuthorUpdateValidator;

        [HttpGet]
        public async Task <ActionResult<IEnumerable<Author>>> GetAll([FromQuery] string? name)
        {
            if(name is not null)
            {
                var authorsWiteName = await _service.GetAllAuthorsWithNameAsync(name);
                var authorsDtoWithName = authorsWiteName.Select(a => new AuthorForReadWithCount(a.Id, a.Name, a.DateOfBirth, a.Books.Count()));
                return Ok(authorsDtoWithName);
            }
